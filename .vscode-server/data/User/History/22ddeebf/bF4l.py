...
# ---------------------------------------------------------
# 3. DG0 damage field and material parameters (HOMOGENEOUS - independent of damage)
# ---------------------------------------------------------
V0 = fem.functionspace(msh, ("DG", 0))   # one value per cell
damage_fn = fem.Function(V0)
damage_fn.x.array[:] = damage_cells

# Base material (homogeneous throughout)
E0 = 1.0e9   # Pa
nu = 0.3

# Constant Lamé parameters (homogeneous material)
lambda_const = (E0 * nu) / ((1 + nu) * (1 - 2 * nu))
mu_const     = E0 / (2 * (1 + nu))

lambda_fn = fem.Constant(msh, PETSc.ScalarType(lambda_const))
mu_fn     = fem.Constant(msh, PETSc.ScalarType(mu_const))

print(f"[INFO] Using HOMOGENEOUS material properties (damage-independent)")
print(f"[INFO] Young's modulus E: {E0:.3e} Pa")
print(f"[INFO] Poisson's ratio nu: {nu:.3f}")
print(f"[INFO] Lamé constants: lambda={lambda_const:.3e}, mu={mu_const:.3e}")
...
# ---------------------------------------------------------
out_vtu = base + "_solution_elasticity_homogeneous.vtu"
...
print("[DONE] RVE elasticity with homogeneous material + postprocessing complete.")
