...
with io.XDMFFile(comm, diffusion_xdmf, "r") as xdmf_c:
    # This reads the mesh again (ignored) and then all time steps into the same Function.
    # After the loop, 'c' contains the last time step.
    xdmf_c.read_mesh(msh)  # mesh should be identical

    # Try to read all time steps; last one stays in 'c'
    try:
        while True:
