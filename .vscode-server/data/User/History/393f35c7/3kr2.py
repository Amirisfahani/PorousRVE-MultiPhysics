...
# 0. User input
# ---------------------------------------------------------
base = "rve_output/rve_quad_mesh_Lx1.00_phi50"  # <-- CHANGE if needed
...
# ---------------------------------------------------------
# 4. Read concentration field c(x) from diffusion HDF5
#    (take the last available time step)
# ---------------------------------------------------------
Vc = fem.functionspace(msh, ("CG", 1))
c = fem.Function(Vc, name="c")

# Open the HDF5 file to find all time steps
h5_file = diffusion_xdmf.replace(".xdmf", ".h5")
with h5py.File(h5_file, "r") as f:
    # List all time steps under /Function/c/
    c_group = f["/Function/c"]
    time_steps = sorted(c_group.keys(), key=lambda x: float(x.replace('_', '.')))
    
    if time_steps:
        last_time_step = time_steps[-1]
        c_data = c_group[last_time_step][:]
        c.x.array[:] = c_data.flatten()
        
        if rank == 0:
            print(f"[INFO] Loaded concentration c(x) from diffusion file (time step: {last_time_step}).")
    else:
        if rank == 0:
            print(f"[WARNING] No time steps found in {h5_file}")
