...
base = "rve_output/rve_quad_mesh_Lx1.00_phi50"  # <-- CHANGE THIS
...
# Make D big enough so that changes in c are clearly visible
D_solid = 0.05      # <<< CHANGED: was 1.0e-10, now much larger (dimensionless-ish)
alpha_D = 5.0       # <<< CHANGED: smaller amplification, still faster in damaged zones
...
# ---------------------------------------------------------
# Choose t_end so diffusion length ~ sqrt(D * t_end) is O(0.1–0.2) for Lx ~ 1
dt    = 1.0e-3   # time step
t_end = 0.2      # <<< CHANGED: shorter total time but with big D we see evolution
...
# ---------------------------------------------------------
diff_outfile = base + "_hydrogen_diffusion_visible.xdmf"  # <<< CHANGED name
...
with io.XDMFFile(MPI.COMM_WORLD, diff_outfile, "w") as xdmf_c:
    ...
    for n in range(1, num_steps + 1):
        ...
        # Diagnostics: show min/max concentration so you SEE the evolution
