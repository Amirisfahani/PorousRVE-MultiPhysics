...
u_trial = ufl.TrialFunction(Vu)  # trial function for bilinear form
v = ufl.TestFunction(Vu)         # test function
...
def eps(u_):
    grad_u = ufl.grad(u_)
    # For 2D: construct symmetric strain tensor manually
...
# For zero RHS, use a simple form that evaluates to zero
...
if MPI.COMM_WORLD.rank == 0:
    ...
    # ct.array is a flat array of node indices for all cells
    num_cells = msh.topology.index_map(tdim).size_local
    num_nodes_per_cell = ct.num_nodes
    # Verify and reshape correctly
    total_indices = ct.array.shape[0]
    expected_total = num_cells * num_nodes_per_cell
    print(f"[DEBUG] ct.array.shape={ct.array.shape}, num_cells={num_cells}, num_nodes_per_cell={num_nodes_per_cell}")
    print(f"[DEBUG] expected_total={expected_total}, actual_total={total_indices}")
    
    cells_dolfinx = ct.array.reshape(num_cells, num_nodes_per_cell)
    points_out = msh.geometry.x
    uh_arr = uh.x.array.reshape(-1, tdim)

    # Guess cell type: quad if 4 nodes per cell, else triangle
