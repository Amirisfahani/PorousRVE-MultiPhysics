...
with io.XDMFFile(MPI.COMM_WORLD, mesh_file_xdmf, "r") as xdmf:
    # Your XDMF from meshio typically has <Grid Name="Grid">
...
# Make sure connectivity is built for later
...
# DG0 = piecewise constant over cells
V0_elem = FiniteElement("DG", msh.ufl_cell(), 0)
V0 = fem.FunctionSpace(msh, V0_elem)
...
# Left BC: u = (0, 0)
u_left_val = np.array((0.0, 0.0), dtype=PETSc.ScalarType)
bc_left = fem.dirichletbc(u_left_val, dofs_left, Vu)

# Right BC: u = (u0, 0)
u_right_val = np.array((u0, 0.0), dtype=PETSc.ScalarType)
